...
Found and fixed independently in two places — this session, and in parallel by a collaborator (mrunalshende05@gmail.com) who pushed a near-identical fix to the shared repo before this one landed. The version that shipped gives the initial solve `total_budget - slice_budget`, reserving exactly one slice's worth of time for at least one repair round. A separate hypothesis (per-round `build_model()` rebuild overhead) was checked and ruled out before landing on this cause — rebuild costs ~3ms out of a ~35–50ms round, not the dominant cost.
